...
# Operator token gating the full /healthz body + /diag (G-8). Reuse if present.
HEALTHZ_TOKEN=""
if [ -f "$ENV_FILE" ]; then
  HEALTHZ_TOKEN=$(grep -E '^HEALTHZ_TOKEN=' "$ENV_FILE" | head -1 | cut -d= -f2- || true)
fi
[ -z "$HEALTHZ_TOKEN" ] && HEALTHZ_TOKEN=$(openssl rand -hex 16)
...
step "Writing $ENV_FILE"
{
  ...
  echo "PORT=8080"
  echo "HOST=127.0.0.1"   # bind to loopback — nginx (same host) is the only ingress (G-7)
  echo "LOG_LEVEL=info"
  echo "HEALTHZ_TOKEN=$HEALTHZ_TOKEN"   # full /healthz + /diag require X-Healthz-Token (G-8)
...
step "Verifying"
sleep 2
HEALTH=$(curl -fsS --max-time 8 -H "X-Healthz-Token: $HEALTHZ_TOKEN" http://127.0.0.1:8080/healthz 2>/dev/null || true)
if [ -n "$HEALTH" ]; then
